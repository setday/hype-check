...
3. Run CausalPFN inference (placeholder)
...
from src.metrics.uplift_metrics import QiniMetric
...
def test_s_learner(train_dataset: UpliftDataset, val_dataset: UpliftDataset) -> Dict[str, Any]:
    ...
    with torch.no_grad():
        for batch in val_loader:
            output = model.forward(features=batch["features"])
            cate_pred = output["cate_pred"]
            qini_metric.update(
                cate_pred=cate_pred,
    ...
    qini = qini_metric.compute().item()
    logger.info(f"S-Learner Qini: {qini:.4f}")

    return {"model": "s_learner", "qini": qini}


def test_causalpfn(val_dataset: UpliftDataset) -> Dict[str, Any]:
    """
    Test CausalPFN (frozen model, no training).
    """
    ...
    with torch.no_grad():
        for batch in val_loader:
            try:
                ...
                logger.warning(f"CausalPFN inference not yet implemented: {e}")
                # Return placeholder results
                return {"model": "causalpfn", "qini": float("nan"), "note": "placeholder"}
...
def main(args):
    ...
    if "causalpfn" in args.models:
        result = test_causalpfn(val_dataset)
        results.append(result)
